...
## Anotação de tipos
 ...
 - callout(Experimente): Apague as barras de comentário da linha de código abaixo para ver o erro. Quando terminar, coloque as barras de comentário novamente.
 */
//let mysteryConstant
/*:
 O erro `Type annotation missing in pattern` (anotação de tipo ausente no padrão) significa que a linguagem Swift não consegue inferir (descobrir a partir das informações disponíveis) qual é o tipo da constante.
 
 Às vezes, é melhor que a linguagem Swift não use a inferência de tipos, pois ela pode não identificar o tipo que você queria, como no cálculo com os tipos `Double` e Int que você tentou fazer.
