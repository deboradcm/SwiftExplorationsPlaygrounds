 ...
 */
import Foundation
//: Um dos tipos da estrutura Foundation é `Date`, que representa um momento específico no tempo. Para criar uma `Date` representando _este exato momento_, basta usar `Date()`:
let today = Date()
/*:
 Você verá a data e o horário atuais na barra lateral de resultados.

 - callout(Nota): Diferentemente das strings e dos números, não é possível criar uma `Date` a partir de um literal.
 */
/*:
 - callout(Experimente): 
...
[Anterior](@previous)  |  página 11 de 13  |  [Na sequência: Conclusão](@next)
 */